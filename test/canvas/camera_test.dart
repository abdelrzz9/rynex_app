...
void main() {
  group('Camera', () {
    ...
    test('screenToWorld inverts transform', () {
      const camera = Camera(zoom: 2.0, pan: Offset(100, 50));
    ...
    test('screenToWorldRect inverts transform for rect', () {
      const camera = Camera(zoom: 2.0, pan: Offset(100, 50));
      final worldRect = camera.screenToWorldRect(const Rect.fromLTWH(100, 50, 200, 100));
    ...
    test('worldToScreenRect applies transform for rect', () {
      const camera = Camera(zoom: 2.0, pan: Offset(100, 50));
      final screenRect = camera.worldToScreenRect(const Rect.fromLTWH(0, 0, 100, 50));
    ...
    test('toMatrix4 and fromMatrix4 roundtrip', () {
      const camera = Camera(zoom: 2.5, pan: Offset(100, 200));
    ...
    test('lerp interpolates between two cameras', () {
      const a = Camera();
      const b = Camera(zoom: 3.0, pan: Offset(100, 50));
    ...
    test('screenToWorld and worldToScreen are inverses', () {
      const camera = Camera(zoom: 1.5, pan: Offset(50, 25));
      const original = Offset(123.456, 789.012);
